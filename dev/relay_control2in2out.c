...
THREAD(relay_control_thread, arg)
{
	...
	if(THISINFO)printf("Start relay_control_thread 1,io_out[0]=0x%X\r\n",io_out[0]);
	//if(THISINFO)NutSleep(5000);
	if(THISINFO)printf("Start relay_control_thread 2,io_out[0]=0x%X\r\n",io_out[0]);
	for(;;) {
...
int relay_init(NUTDEVICE * dev)
{
	...
	wdtfile = fopen("resetctl", "w+b");
	_ioctl(_fileno(wdtfile), GET_RESET_TYPE, &type);

	if(type&AVR_EXT_RESET) {
		if(THISINFO)printf("Extern Reset\r\n"); 
		...
		if(THISINFO)printf("Wdt Reset\r\n");
		//char * pmem = &relay_dcb;
		//pmem += 2;
		//memset(pmem,0,sizeof(relay_dcb)-2);  //复位延时参数，继电器不改变
	}
	if(THISINFO)printf("io_out=0x%X\r\n",io_out[0]);
	//if(THISINFO)NutSleep(5000);
	relay_dcb.input[0] = lunch_in_lunch_out(io_out[0]);
