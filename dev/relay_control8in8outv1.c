...
uint8_t relay_PortIn(void)
{
	...
	GpioPinConfigSet(AVRPORTB, 4, GPIO_CFG_PULLUP);
	GpioPinConfigSet(AVRPORTB, 5, GPIO_CFG_PULLUP);
	GpioPinConfigSet(AVRPORTB, 6, GPIO_CFG_PULLUP);
	GpioPinConfigSet(AVRPORTB, 7, GPIO_CFG_PULLUP);

	GpioPinSetHigh(AVRPORTF, 5);
	NutSleep(1);
	...
	if(GpioPinGet(AVRPORTB,4)) {
		in |= (1<<4);
	}
	if(GpioPinGet(AVRPORTB,5)) {
		in |= (1<<5);
	}
	if(GpioPinGet(AVRPORTB,6)) {
		in |= (1<<6);
	}
	if(GpioPinGet(AVRPORTB,7)) {
		in |= (1<<7);
	}
